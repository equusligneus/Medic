...
public class UI_LifeCount : MonoBehaviour
{
    ...
    private void OnLivesChanged()
    {
        ...
        {
            var bg = lives[i].style.backgroundImage;
            var image = bg.value;
            image.sprite = i < currentLives.Get() ? lifeFull : lifeEmpty;
            bg.value = image;
            lives[i].style.backgroundImage = bg;
        }
    }
    ...
    private void OnMaxLivesChanged()
	{
        ...
        foreach(var child in parent.Children())
            lives.Add(child.Q<VisualElement>("Life"));
